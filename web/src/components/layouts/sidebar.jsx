"use client";

import { useEffect, useState } from "react";
import Link from "next/link";
import { useParams, useSelectedLayoutSegment } from "next/navigation";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";
import {
  Terminal,
  FolderOpen,
  Archive,
  Puzzle,
  Settings,
  Users,
  ChevronLeft,
  ChevronRight,
  Globe,
  UserCheck,
} from "lucide-react";

const sidebarItems = [
  { id: "console", label: "Console", icon: Terminal },
  { id: "config", label: "Server Config", icon: Settings },
  { id: "players", label: "Player List", icon: Users },
  { id: "worlds", label: "Worlds", icon: Globe },
  { id: "files", label: "File Manager", icon: FolderOpen },
  { id: "mods", label: "Mods", icon: Puzzle },
  { id: "plugins", label: "Plugins", icon: Puzzle },
  { id: "backup", label: "Backup", icon: Archive },
  { id: "access", label: "Access Management", icon: UserCheck },
];

export function Sidebar() {
  const [isCollapsed, setIsCollapsed] = useState(false);
  const params = useParams();
  const segment = useSelectedLayoutSegment();

  const activeTab = sidebarItems.some((item) => item.id === segment)
    ? segment
    : "console";
  const serverId = params?.id;

  useEffect(() => {
    const width = isCollapsed ? "4rem" : "16rem";
    if (typeof document !== "undefined") {
      document.documentElement.style.setProperty("--sidebar-width", width);
    }
  }, [isCollapsed]);

  useEffect(() => {
    if (typeof window !== "undefined") {
      if (window.innerWidth < 768) {
        setIsCollapsed(true);
      }
    }
  }, []);

  return (
    <div
      className={cn(
        "bg-background border-r transition-all duration-300 flex flex-col fixed left-0 top-0 h-screen overflow-y-auto z-40",
        isCollapsed ? "w-16" : "w-64"
      )}
      aria-label="Sidebar"
    >
      {/* Sidebar Header */}
      <div className="py-4 px-4 h-18 border-b flex items-center justify-between">
        {!isCollapsed && (
          <h3 className="font-semibold text-sm text-muted-foreground">
            SERVER PANEL
          </h3>
        )}
        <Button
          variant="ghost"
          size="icon"
          onClick={() => setIsCollapsed(!isCollapsed)}
          className="h-8 w-8"
        >
          {isCollapsed ? (
            <ChevronRight className="h-4 w-4" />
          ) : (
            <ChevronLeft className="h-4 w-4" />
          )}
        </Button>
      </div>

      <nav className="flex-1 p-2">
        <ul className="space-y-1">
          {sidebarItems.map((item) => {
            const Icon = item.icon;
            return (
              <li key={item.id}>
                <Button
                  variant={activeTab === item.id ? "secondary" : "ghost"}
                  className={cn(
                    "w-full justify-start gap-3 h-10 cursor-pointer",
                    isCollapsed && "justify-center px-2"
                  )}
                  asChild
                >
                  <Link href={`/panel/${serverId}/${item.id}`} prefetch>
                    <Icon className="h-4 w-4 flex-shrink-0" />
                    {!isCollapsed && (
                      <span className="truncate">{item.label}</span>
                    )}
                  </Link>
                </Button>
              </li>
            );
          })}
        </ul>
      </nav>
    </div>
  );
}
